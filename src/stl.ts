...
export function normalizeLongestEdgeMm(value: unknown) {
  if (value == null || value === '') return DEFAULT_LONGEST_EDGE_MM;
  const n = Number(value);
  if (!Number.isFinite(n) || n < MIN_LONGEST_EDGE_MM || n > MAX_LONGEST_EDGE_MM) {
    throw new Error(
      `longestEdgeMm must be between ${MIN_LONGEST_EDGE_MM} and ${MAX_LONGEST_EDGE_MM}`,
    );
  }
  return n;
}

export async function glbToBinaryStl(
  glb: Uint8Array,
...
export async function writeGlbFile(filePath: string, glb: Uint8Array) {
